 ...
 * 
 * Implementation of the email provider interface for Gmail accounts.
 * Uses IMAP with OAuth2 for receiving mail and SMTP for sending.
 */
export class GmailProvider extends BaseEmailProvider {
    ...
    try {
      ...
      if (options.auth.type === 'oauth2') {
        if (!options.auth.accessToken) {
          this.setLastError(new Error('OAuth2 access token is required for Gmail'));
          return false;
        }
      ...
      } else if (options.auth.type === 'app_password') {
        ...
        // Configure SMTP transporter with app password
        this.smtpTransporter = nodemailer.createTransport({
          host: 'smtp.gmail.com',
          port: 465,
          secure: true,
        ...
        this.setLastError(new Error('Unsupported authentication method for Gmail'));
        return false;
      }
      ...
      this.imapClient.getBoxes((err, boxes) => {
        ...
        const processBoxes = (boxObject: any, path = '') => {
          for (const name in boxObject) {
            ...
            // Gmail uses '[Gmail]' as a prefix for system folders
            if (name === '[Gmail]' && boxObject[name].children) {
  ...
  async deleteMessage(id: string): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      ...
      // Gmail doesn't use the standard delete flag, it uses the Trash folder
      // So we'll move the message to the Trash folder instead
      this.moveMessage(id, '[Gmail]/Trash').then(resolve).catch(() => resolve(false));
    });
  }
